from datetime import datetime #для ортимання дати та часу від системи
import os #для очищеня консолі
import requests #для використання UML запитів
import csv #для роботи з файлами csv
import re #для роботи з регулярними виразами
from bs4 import BeautifulSoup #для отримання html сторінок (парсингу)

from tkinter import * #для створеня об'єктів графічного інтефейсу
...
def get_html(url):#отримання сторінки
	r = requests.get(url)
	return r.text

def get_last_page(html):#виділеня порядкового номеру останньої сторінки
...
def write_csv(data):#запис до файлу
	try:
...
def get_page_data(html):#виділення інформації по оголошеням на поточній сторінці
	soup = BeautifulSoup(html, 'lxml')
	ads = soup.find('table', class_ = 'fixed offers breakword ').find_all('tr', class_ = 'wrap')
	
	for ad in ads:
		#title, price, location, url, time
		try:#отримання заголовку оголошеня
			title = ad.find('strong').text	
		except:
			title = ''
	
		try:#отримання ціни оголошеня 
		...
		try:#отримання місця розташування оголошеня
		...
		try:#отриманя дати розміщеня оголошеня
		...
		try:#отримання посилання на оголошення
			url = ad.find('a', class_ = 'marginright5 link linkWithHash detailsLink').get('href')
			url = re.sub(';promoted','',url)
		except:
			url = ''
		
		data={'title':title, 'price':price, 'location':location, 'time':time, 'url':url}# формування словника для поточного оголошення
		write_csv(data)#виклик функції для запису в файл поточного оголошення
		
	return int(len(ads))
	
def main(console):
	Start_working = datetime.now()#отримання дати початку роботи

	ads_sum = 0#лічильник оголошень
	last_page = get_last_page(get_html(url_base+url_query))
	
	for i in range(1, last_page+1):#цикл проходу по сторінках з оголошеннями
		url_gen = url_base + url_query + url_page + str(i)#формуваня URL на поточну сторінку
		html = get_html(url_gen)#отримання html коду поточноъ сторынки
		ads_sum+=get_page_data(html)#отримання данних з поточноъ сторіник

	Stop_working = datetime.now()#отримання часу закінченя роботи
...
output = Text(root, bg="lightblue", font="Arial 12", width=35, height=10)#створеня віджету для виведеня тексту
